...
#### 共通化との相乗効果
- `MainLayout` の `v-app-bar color="primary"` は CSS変数を参照するため、テーマ変更で**全ページのAppBarの色が一括変更**される
- コンポーネントを共通化していないと、テーマ変更の効果が散在する直書きコードに届かない
- **「コンポーネント共通化」×「テーマ変数」= 1箇所の変更がアプリ全体に伝わる設計**
